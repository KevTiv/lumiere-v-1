...
const schemaPath = new URL('schema/module-draft.schema.json', packageRoot);
const schemaText = schemaOnlyInput
  ? await readFile(schemaPath, 'utf8')
  : execFileSync('cargo', ['run', '--locked', '--quiet', '-p', 'lumiere-presentation-core', '--bin', 'presentation-schema'], {
      cwd: fileURLToPath(root), encoding: 'utf8', maxBuffer: 4 * 1024 * 1024,
    });
const schema = JSON.parse(schemaText);
const normalizedSchema = `${JSON.stringify(schema, null, 2)}\n`;
const types = await compile(schema, 'ModuleDraft', {
...
for (const [url, content] of [
  ...
  [new URL('src/generated/module-draft.ts', packageRoot), types],
]) {
  if (check) {
