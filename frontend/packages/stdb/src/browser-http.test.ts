...
test("mutation bridge only uses the typed browser operation transport", async () => {
  assert.ok(SESSION_OPERATION_NAMES.length > 0)
  for (const fileName of ["crm.ts", "form-config.ts", "settings-admin.ts"]) {
    ...
    assert.doesNotMatch(source, /\bstdbBrowserCompatCall\s*\(/)
  }
})
