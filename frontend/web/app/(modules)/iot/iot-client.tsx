...
type Loaded = Omit<IotClientProps, 'organizationId'> & {
  organizationId: number;
};
...
const telemetryBatchForm: FormConfig = {
  ...
  description:
    'Provide an array of telemetry readings with sensor_type, value, unit, and quality.',
...
function parseTelemetryReadings(value: unknown) {
  ...
  return parsed.map((entry) => {
    if (!entry || typeof entry !== 'object')
      throw new Error('Each reading must be an object');
    const row = entry as Record<string, unknown>;
    const value = Number(row.value);
    if (!Number.isFinite(value))
      throw new Error('Each reading needs a numeric value');
    return {
...
function IotClientLoaded({
  ...
  const { data: pairingTokens = [] } = useIotPairingTokens(
    orgId,
    initialPairingTokens,
  );
  ...
  const genPairing = useGenerateHubPairingToken(orgId, operatingCompanyId);
  const registerHub = useRegisterIotHub(orgId, operatingCompanyId);
  const registerDevice = useRegisterIotDevice(orgId, operatingCompanyId);
  ...
  const [deviceRow, setDeviceRow] = useState<Record<string, unknown> | null>(
    null,
  );
  ...
  const pendingActions = useMemo(
    () =>
      actions.filter(
        (r) => str((r as Record<string, unknown>).status) === 'Pending',
      ).length,
    [actions],
  );
  ...
  const config = useMemo(
    () => ({
      ...moduleConfigBase,
      tabs: withDashboardSections(moduleConfigBase, liveSections).tabs.map(
        (tab) => {
    ...
    if (action === 'registerIotHub') {
      await registerHub.mutateAsync({
        ...
        firmware_version: formData.firmware_version
          ? String(formData.firmware_version)
          : null,
        metadata: null,
      });
      ...
      if (!Number.isFinite(hubId) || hubId <= 0)
        throw new Error(t('iot.forms.errors.invalidHubId'));
  ...
  const handleDeviceRowSubmit = async (formData: Record<string, unknown>) => {
    ...
    if (op === 'link_location') {
      ...
      if (!Number.isFinite(pid) || pid <= 0)
        throw new Error(t('iot.forms.errors.invalidPosId'));
      ...
      if (!Number.isFinite(v))
        throw new Error(t('iot.forms.errors.invalidTelemetryValue'));
  ...
  const handleIotModalSubmit = async (formData: Record<string, unknown>) => {
    ...
    try {
      ...
      if (iotModal.type === 'createAction') {
        ...
        await Promise.all(
          ids.map((deviceId) =>
            recordTelemetryBatch.mutateAsync({ deviceId, readings }),
          ),
        );
      ...
      <FormModal
        ...
        onSubmit={async (fd) => {
          ...
          if (!Array.isArray(parsed))
            throw new Error(t('iot.forms.errors.jsonNotArray'));
          const detected = parsed.map((item) => {
            if (!item || typeof item !== 'object')
              throw new Error(t('iot.forms.errors.badSyncEntry'));
