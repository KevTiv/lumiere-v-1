...
function extractWebReducers(): {
  ...
  try {
    const output = execSync(
      `rg -o '/api/call/[a-z0-9_?]+' ${WEB_SRC} --glob '*.ts' --glob '*.tsx' | sed 's|.*/api/call/||' | sed 's/?.*//' | sort -u`,
  ...
  const files = [
    ...globSync('**/*.ts', { cwd: WEB_SRC, absolute: true }),
    ...globSync('**/*.tsx', { cwd: WEB_SRC, absolute: true }),
  ]
...
function categorizeByModule(reducerName: string): string {
  ...
  const prefixMap: Record<string, string> = {
    ...
    update_uom: 'inventory',
    delete_uom: 'inventory',
    ...
    update_quality_check: 'inventory',
    delete_quality_check: 'inventory',
    pass_quality_check: 'inventory',
    fail_quality_check: 'inventory',
    create_quality_alert: 'inventory',
    update_quality_alert: 'inventory',
    delete_quality_alert: 'inventory',
    assign_quality_alert: 'inventory',
    cancel_quality_alert: 'inventory',
    create_replenishment_rule: 'inventory',
    update_replenishment_rule: 'inventory',
    delete_replenishment_rule: 'inventory',
    trigger_replenishment: 'inventory',
    ...
    update_picking_wave: 'inventory',
    delete_picking_wave: 'inventory',
    confirm_picking_wave: 'inventory',
    process_picking_wave: 'inventory',
