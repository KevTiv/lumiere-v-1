...
test.describe("Accounting module e2e", () => {
  ...
  test("creates tax, budget, payment term, and payment term line", async ({ page }) => {
    ...
    await expect(page.getByText(budgetName).first()).toBeVisible()
  ...
  test("creates fiscal year then accounting period", async ({ page }) => {
    ...
    await fillField(page, "dateFrom", isoDateTimeLocal(2026, 1, 1, 0, 0))
    await fillField(page, "dateTo", isoDateTimeLocal(2026, 12, 31, 23, 59))
    await chooseFirstOption(page, "fiscalYearType")
    await submitForm(page, "new-fiscal-year")
    await expect(page.getByText(fyName).first()).toBeVisible()
    ...
    await chooseFirstOption(page, "fiscalYearId")
    await fillField(page, "dateFrom", isoDateTimeLocal(2026, 1, 1, 0, 0))
    await fillField(page, "dateTo", isoDateTimeLocal(2026, 3, 31, 23, 59))
