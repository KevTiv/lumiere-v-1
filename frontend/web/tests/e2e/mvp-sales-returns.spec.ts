...
test.describe("MVP sales returns (RMA)", { tag: "@p0" }, () => {
  test("creates partial return, receives goods, and posts credit note", async ({ page }) => {
    ...
    await Promise.all([
      page.waitForResponse(
        (res) => matchesTypedOperationResponse(res, "create_return_order") && res.ok(),
        { timeout: 30_000 },
      ),
    ...
    await Promise.all([
      page.waitForResponse(
        (res) => matchesTypedOperationResponse(res, "confirm_return_order") && res.ok(),
        { timeout: 30_000 },
      ),
    ...
    await Promise.all([
      page.waitForResponse(
        (res) =>
          matchesTypedOperationResponse(res, "create_credit_note_from_return_order") && res.ok(),
        { timeout: 30_000 },
      ),
      submitForm(page, "create-invoice-from-sale-order"),
    ])
