  ...
  test("manages a phone identity and contact role from the CRM record", async ({ page }) => {
    ...
    await Promise.all([
      page.waitForResponse(
        (response) => matchesOperationResponse(response, "create_contact_identity") && response.ok(),
        { timeout: 30_000 },
      ),
    ...
    await Promise.all([
      page.waitForResponse(
        (response) => matchesOperationResponse(response, "assign_contact_role") && response.ok(),
        { timeout: 30_000 },
      ),
    ...
    await Promise.all([
      page.waitForResponse(
        (response) => matchesOperationResponse(response, "end_contact_role") && response.ok(),
        { timeout: 30_000 },
      ),
