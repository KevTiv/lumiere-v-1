...
def validate_manifest(manifest: dict[str, Any], expected_table_count: int) -> None:
    ...
    # expected count. The production C0 invocation uses the default 458-table
    # count and must contain the complete, closed platform allowlist.
    if expected_table_count == 458 and platform_tables != PLATFORM_GLOBAL_TABLES:
        fail(
    ...
    if expected_table_count == 458 and summary is None:
    ...
    if summary is not None:
        ...
        if expected_table_count == 458 and summary_names != PLATFORM_GLOBAL_TABLES:
...
def main() -> None:
    ...
    parser.add_argument("--expected-table-count", type=int, default=458)
    parser.add_argument(
        "--schema-ir-source",
        type=Path,
