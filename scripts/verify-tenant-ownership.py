...
All 465 relations in the ERP manifest are organization-routed: 460
...
EXPECTED_TABLE_COUNT = 465
APPLICATION_RELATION_COUNT = 460
...
def validate_manifest(manifest: dict[str, Any], expected_table_count: int) -> None:
    ...
    if expected_table_count == EXPECTED_TABLE_COUNT:
        ...
        if len(application) != APPLICATION_RELATION_COUNT:
            fail(
                "C0 requires 460 application relations + 5 protocol relations, "
...
def main() -> None:
    ...
    parser.add_argument("--expected-table-count", type=int, default=465)
    parser.add_argument(
        "--schema-ir-source",
        type=Path,
