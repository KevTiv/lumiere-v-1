...
All 501 relations in the ERP manifest are organization-routed: 496
...
EXPECTED_TABLE_COUNT = 501
APPLICATION_RELATION_COUNT = 496
...
def validate_manifest(manifest: dict[str, Any], expected_table_count: int) -> None:
    ...
    if expected_table_count == EXPECTED_TABLE_COUNT:
        ...
        if len(application) != APPLICATION_RELATION_COUNT:
            fail(
                "C0 requires 496 application relations + 5 protocol relations, "
...
def main() -> None:
    ...
    parser.add_argument("--expected-table-count", type=int, default=501)
    parser.add_argument(
        "--schema-ir-source",
        type=Path,
