...
function validate(schema, policyDocument, resourceRegistry) {
  if (!Array.isArray(schema.tables) || schema.tables.length !== 479) {
    throw new Error(`expected schema manifest with 479 tables, found ${schema.tables?.length ?? "none"}`);
  }
  ...
  if (schema.ownership_summary?.verified !== true
      || schema.ownership_summary.erp_owned_count !== 479
      || schema.ownership_summary.platform_global_count !== 0) {
    throw new Error("schema manifest must carry verified C0 ownership totals (479 organization + 0 platform)");
  }
  ...
  if (platform.length !== PLATFORM_GLOBAL_TABLES.size || organization.length !== 479) {
    throw new Error(`C0 ownership split must be 479 organization + 0 platform, got ${organization.length} + ${platform.length}`);
  }
...
if (checkOnly) {
  ...
  console.log(`C1 storage policy check passed: ${policyDocument.policies.length}/479 tables`);
} else {
  fs.writeFileSync(policyPath, `${JSON.stringify(policyDocument, null, 2)}\n`);
  console.log(`Wrote ${policyPath}`);
  console.log(`C1 storage policy bootstrap passed: ${policyDocument.policies.length}/479 tables`);
}
