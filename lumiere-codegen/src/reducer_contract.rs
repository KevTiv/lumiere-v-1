...
pub fn run(paths: &Paths) -> Result<()> {
    ...
    let identity_text = read_to_string(&paths.contract_operation_ids_json)?;
    let identity_manifest: OperationIdentityManifest = serde_json::from_str(&identity_text)
        .with_context(|| {
            format!(
                "parse contract operation identities {}",
                paths.contract_operation_ids_json.display()
            )
        })?;
    if identity_manifest.schema_version != 1 {
        bail!(
            "unsupported contract operation identity version {}",
            identity_manifest.schema_version
        );
    }
    let identity_names = identity_manifest
        .operations
        .keys()
        .map(String::as_str)
        .collect::<BTreeSet<_>>();
    ...
    for (name, id) in &identity_manifest.operations {
        if !id.strip_prefix("erp.").is_some_and(|key| {
            !key.is_empty()
                && key
                    .bytes()
    ...
    for reducer in reducers {
        entries.push(parse_reducer(
            ...
            identity_manifest
                ...
                .expect("identity coverage checked"),
        )?);
    }
...
fn parse_reducer(
    ...
    Ok(ReducerEntry {
        name,
        contract_operation_id: contract_operation_id.to_owned(),
        params,
        client_input,
        server_context,
        wire_arguments,
...
fn emit_rust_contract(manifest: &ReducerManifest) -> String {
    ...
    for (index, reducer) in manifest.reducers.iter().enumerate() {
        ...
        writeln!(
            out,
            "    ReducerContract {{ name: {:?}, contract_operation_id: {:?}, params: PARAMS_{index}, lifecycle: {:?}, exposure: Exposure::{}, organization_position: {}, company_position: {}, unscoped_reason: {} }},",
            reducer.name,
            reducer.contract_operation_id,
            reducer.lifecycle,
