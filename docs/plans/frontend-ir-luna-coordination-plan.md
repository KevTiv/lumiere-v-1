...
**Validation in progress:** canonical Rust validator 13/13 and schema drift 2/2 passed; frontend core runtime/decoder tests 5/5 and typecheck passed; collection/detail host tests 5/5 passed; generated TypeScript drift, opaque-record ratchet and immutable browser transport passed. UI/web typechecks and full Rust-to-schema-to-TypeScript drift also passed locally. API tests remain pending; initial CI caught a String/reference comparison in projection, corrected before rerunning the gate. Tests use a local mock STDB transport; a deployed database/browser interaction proof has not been run.
