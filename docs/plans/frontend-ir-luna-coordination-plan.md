...
## Wave 3a: canonical saved-draft contract foundation

This stacked increment implements the database owner needed before API/editor save/reopen. `presentation_module` is a personal organization/actor-scoped head; `presentation_module_version` is immutable draft history. `save_presentation_module` derives the owner and module key, checks membership/write permission, compares the expected revision, and records head/version/audit changes atomically. Limits are 100 heads per owner, 256 snapshots per module and 64 KiB per canonical definition.

The database and API share `crates/presentation-core` by path dependency. Persistence uses its strict canonical `ModuleDraft` preparation, preserves all pages/nodes/fields, clears the embedded `baseRevision` to null, and stores revision separately. Saved pins and bindings are untrusted author intent, not capability certification. Every future read/preview/publication consumer must revalidate the current actor's catalog. Generic BFF reducer exposure remains denied until that owner-scoped API is implemented after the contract release.

Luna lanes own database persistence, pure canonical preparation, and persisted test fixtures. The coordinator owns integration, the 465-table ownership census, durable-history policy, generated reconstruction, source provenance, contract candidate and release gates. No frontend save control is enabled against an unreleased contract.

Validation: pure presentation-core tests 17/17 plus schema drift 2/2; storage-policy unit tests 13/13; contract/history/release script tests 30/30; native STDB check including test targets; C0 ownership and C1 storage policy for 465/465 tables; C2 commit coverage; and clean canonical IR/history checks pass. The persisted in-module harness passed against a disposable local STDB database, covering immutable roundtrip/hash/pins, stale/invalid writes, owner and organization isolation, and head/version/audit commit recording. Generated reconstruction includes both tables, orders the head before versions, and checks organization ownership in both apply handlers. The optimized WASM build also passes. This is not a PostgreSQL-to-fresh-STDB reconstruction replay or a browser save/reopen proof.

The publisher now supports `LUMIERE_CONTRACTS_PREPARE_DIR=<new-absolute-directory>` to retain a fully checked local candidate without pushing a remote commit or tag. A companion release and atomic Rust/TypeScript pin update gate the next API save/read and editor save/reopen increment. Publication/activation and sandbox execution remain subsequent milestones.

The companion advanced to v0.3.42 during this wave without changing its canonical application IR. The next local candidate uses v0.3.43 and preserves its reviewed agent-capability entries, rebinding source provenance and regenerating the checksum; strict companion validation still rejects any descriptor that no longer matches the new IR. No capability is added by saving drafts. The local candidate at `/private/tmp/lumiere-presentation-contracts-v0.3.43-r2/repo` is commit `ba25adbdb4a02bb7778e913745aa3c8991accdec`, based on companion v0.3.42. Its default Rust bindings build, TypeScript typecheck/build, generator drift check, generator tests 22/22, Rust codec/capability fixtures 2/2, Node fixtures 9/9 and package dry-run contents check pass. The publisher also preserves companion-owned Cargo test dependencies. At the Wave 3a handoff, the candidate had not been pushed and the application remained pinned to v0.3.40. The Wave 3b record below supersedes that release status.

**Next milestone (Wave 3b):** release and atomically pin the checked contracts, then implement owner-scoped list/read/save endpoints with session identity, trusted placement, full current-catalog validation and explicit transport admission. A denied generated operation must not be bypassed by a direct reducer call or server-token dispatcher. Resolve its reviewed API admission contract before enabling the composer save/reopen and stale-revision UI. Keep publication, activation and harness sandbox execution outside this increment.


## Wave 3b: personal save/reopen integration

Companion PR #11 passed validate, offline IR generation and GitGuardian checks and was merged. The immutable `v0.3.43` tag resolves to `932a121bf3d06a42d7e6b388fd5c4a90e7817bbc`. Rust, the STDB TypeScript package, query hooks and the release compatibility manifest now consume that release together. Its source IR is the checked Wave 3a snapshot at `e451338e9552d8ea949e1e45008e6973ea6308fa`.

The HTTP API exposes personal list, get and save routes under `/presentation/drafts`. Organization and actor come from the authenticated session; private SQL uses server read authority with owner/organization predicates and generated table decoding. Reopen checks canonical JSON, hash, provenance, immutable revision and the current actor-filtered catalog. Save has a dedicated dispatcher that verifies the exact pinned reducer ID/signature and preserves the user token. Generic operation exposure remains denied. A stale revision returns HTTP 409; the response after a successful save reads that exact new revision even if another save advances the head.

The Rust-owned saved-draft HTTP schema generates TypeScript models and response decoders. Revisions remain decimal strings. The pilot composer saves and reopens its `collections-preview` module, preserves complete multi-page definitions while editing its existing title/field controls, and retains local edits on conflict. Actor/organization changes reset scoped state; company changes retain the definition and invalidate preview data. This increment does not add a general module picker, arbitrary components, publication, activation or sandbox execution.

Luna lanes own API/runtime proof, composer/browser proof, and disposable reconstruction proof. The coordinator owns contract releases/pins, canonical HTTP models, shared integration, final checks and the stacked application PR. Local authenticated API fixtures set `LUMIERE_API_BIND_IP=127.0.0.1`; the default deployment bind address remains `0.0.0.0`. Recovery coverage derives its 23-module/460-restore-table census from the pinned manifest and requires a saved head with its current immutable version.

Validation: pinned contract, release, ownership, storage, commit-coverage and route-boundary gates pass. Rust presentation-core tests pass 17/17 plus 3/3 schema drift checks; TypeScript contract tests pass 8/8; web unit tests pass 48/48 and web typecheck passes. API presentation tests pass 26/26, dispatcher tests 18/18 and reconstruction tests 32 passed (one separately gated runtime test ignored); the updated coverage tests pass 5/5. The isolated PostgreSQL manifest matrix passes.

Live HTTP proof passes create/list/reopen/update, exact revision 2 and stale-write HTTP 409. A distinct actor in another organization receives an empty list and 404 for the owner's draft. Three authenticated browser race tests pass with deterministic HTTP mocks, covering pending-save edits, clean-to-dirty delayed reopen, and conflict/reload UX. Their unmocked browser persistence follow-up remains pending until its result is recorded.

The frozen recovery snapshot covers all 23 modules, 1,725 source rows and 203 populated tables. Failure injection after a committed batch was observed; resume and repeat each verify 460 restore tables, 3,455 restored rows and five recreated tables at the same durable watermark. Delete tombstone and batch receipt checks pass. Full-field comparison verifies three presentation heads and five versions, including their canonical JSON/hash and provenance; six foreign-organization source rows are excluded. This closes the Wave 3b PostgreSQL-to-fresh-STDB proof, not the separate C9 eight-lane isolation matrix.

The implementation is committed in `632a4ea27` and published as stacked draft [PR #25](https://github.com/KevTiv/lumiere-v-1/pull/25), based on PR #19. Detailed HTTP, browser and reconstruction evidence lives in `docs/evidence/frontend-ir-wave3b-*.json`. Publication, activation and sandbox execution remain subsequent gates.
