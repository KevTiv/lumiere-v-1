...
- **DG-07 implemented:** generic pattern status updates can no longer perform `reviewed -> promoted`. The dedicated `promote_ai_decision_pattern` reducer revalidates current supporting cases, the immutable DecisionType graduation policy, stored metric thresholds, deterministic implementation versioning, minimum shadow sample count, minimum shadow conformance rate, and defect/incident review evidence before promotion. Promotion evidence is persisted in the reviewed note as a structured audit snapshot.
- **DG-08 implemented:** `DecisionExecutionMode` is part of the versioned graduation policy and a `DecisionResolutionPolicy` now sits above provider routing. `model_primary` preserves the existing path. `deterministic_primary_model_shadow` resolves exactly one promoted applicability-matching pattern, requires its immutable implementation to be registered in-process, returns the deterministic typed decision as live output, and records the model result as zero-authority shadow evidence. `deterministic_only` is also fail-closed and supported. `deterministic_shadow` remains explicitly non-admitted for live routing and directs callers to the DG-05 evaluator rather than silently changing semantics. Deterministic responses bypass model-confidence floors but still obey DecisionType verification and risk-class escalation.
- **DG-09 implemented:** `StdbDriftMonitor` re-evaluates promoted deterministic authority from durable pre/post-promotion evidence: shadow conformance, post-promotion model-shadow disagreement, supporting-case corrections/supersession, applicability drift, required policy evidence, pattern status, and defect/incident reviews scoped only to runs that exercised the deterministic implementation. Downgrades are monotonic (`deterministic_only -> deterministic_primary_model_shadow -> model_primary`), persisted as append-only `graduation_rollback` intelligence events, and applied by the resolver on subsequent requests. Persisted rollback can never re-promote authority.
- **DG-10 implemented:** reviewed deterministic fixtures live under `ai-gateway/fixtures/deterministic-graduation/` and are checksum-locked in the conformance runner. The suite validates fixture schema, tenant metadata, immutable implementation refs, DecisionType alignment, malformed-request expectations, candidate-set changes, historical cases, threshold boundaries, policy/calibration refs, deterministic output validity, numeric tolerance, and gate-disposition parity. CI now runs `deterministic_graduation_conformance_suite` as an explicit gate before the full ai-gateway unit suite. Reviewed fixtures are never regenerated in CI; changing them requires an intentional fixture edit plus reviewed checksum update.
