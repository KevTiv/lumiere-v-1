...
**No generated contract delta expected.** `timesheets-to-validate` exposes validation_status and timesheet_invoice_id (the plain `timesheets` resource does not)
...
| O | Playwright drives the transition through the visible UI action (setup calls allowed only for fixtures) | TODO — `frontend/web/tests/e2e/cov10-project-timesheet-validation.spec.ts` |
