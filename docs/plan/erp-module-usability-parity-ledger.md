...
> **COV-02A status: ACCEPTED; COV-02B status: REVIEW.** The existing authorities and all COV-03..24 fixture gaps are inventoried in [`erp-cov02a-seed-persona-inventory-status.md`](./erp-cov02a-seed-persona-inventory-status.md). The versioned explicit-org manifest, seven named personas, deterministic provisioner, and scoped health readback are implemented in [`erp-cov02b-first-org-fixture-status.md`](./erp-cov02b-first-org-fixture-status.md). COV-02 remains open pending COV-02C disposable-stack execution, persona/permission proof, IoT baseline, and independent rerun.
