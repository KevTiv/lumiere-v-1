...
| ID | Status | Est. | Depends | Primary ownership | Bounded objective | Acceptance gate |
| --- | --- | ---: | --- | --- | --- | --- |
| `GOV-00` | REVIEW | 1.5 | BASE-02 | gateway route/executor adapter | Route one low-risk skill through the authenticated BFF into the canonical admitted `GovernedProgramExecutor`, require a durable run, and persist its observable trace. Do not resurrect the retired provider-driven direct loop or add another executor. | `report_analysis` receives server-derived organization/actor/company authority, reaches `run_skill_admitted`, and persists its run/ordered steps. **Evidence (2026-09-24):** implementation `9de0ee4cc`; gateway 584 passed/6 prerequisite-ignored/0 failed, frontend unit 80 passed/0 failed, typecheck passed. [`GOV-00 status`](./erp-harness-gov00-status.md). Live authenticated HTTP-to-SpacetimeDB run proof remains required before `ACCEPTED`. |
