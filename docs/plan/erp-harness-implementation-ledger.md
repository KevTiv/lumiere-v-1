...
| ID | Status | Est. | Depends | Primary ownership | Bounded objective | Acceptance gate |
| --- | --- | ---: | --- | --- | --- | --- |
| `GOV-00` | ACCEPTED | 1.5 | BASE-02 | gateway route/executor adapter | Route one low-risk skill through the authenticated BFF into the canonical admitted `GovernedProgramExecutor`, require a durable run, and persist its observable trace. Do not resurrect the retired provider-driven direct loop or add another executor. | `report_analysis` receives server-derived organization/actor/company authority, reaches `run_skill_admitted`, and persists its run/ordered steps. **Evidence (2026-09-25):** acceptance repair `37f04df87`; authenticated Chromium → Next.js BFF → gateway execution passed in the isolated live stack; durable run `17` recorded the signed-in actor, org `264`, company `267`, `step_count=3`, three ordered steps, succeeded capability execution, typed decision event, and observed precedent. Missing trusted headers returned 403 and forged JSON authority returned 422. Gateway 596 passed/6 prerequisite-ignored/0 failed; frontend unit 80 passed/0 failed; typecheck passed. [`GOV-00 status`](./erp-harness-gov00-status.md). Reviewer: Codex coordinator evidence review (not human approval). |
| `GOV-00A` | ACCEPTED | 1.5 | GOV-00, BASE-02 | provider completion/termination metadata + thinking policy | Normalize provider completion termination (`done_reason`/finish reason), content presence and safe usage metadata; discover or configure model thinking controls; classify thinking-budget starvation distinctly from malformed structured output and provider failure; support at most one policy-enabled, separately spend-admitted non-thinking fallback linked to the original attempt; and encode Ollama multi-outcome reasoning as one closed typed union. Raw model-authored chain-of-thought is never persisted, emitted as provenance or treated as authority. | Provider fixtures distinguish success, length-before-content, malformed schema, unsupported thinking control and transport/provider failure. Raw `message.thinking` is absent from durable records and logs; only bounded safe metadata may remain. Any fallback creates a separately budgeted/settled attempt with causal linkage, cannot run after budget exhaustion and never loops silently. Unadmitted union outcomes fail closed. One live Ollama certification produces either a valid schema-constrained outcome or the exact typed exhausted terminal state, with `done_reason` inspectable. **Evidence (2026-09-25):** implementation `e460e7e1c`; gateway 606 passed/7 prerequisite-ignored/0 failed; provider fixtures 13 passed/1 live-ignored; intelligence adapters 22 passed; spend admission 14 passed; spend parsing/accounting 12 passed. The ignored live certificate was executed separately with local `gemma4:e2b-mlx` and passed at one unchanged 256-token ceiling: thinking starvation remained a typed `length` terminal condition and the single closed-union `think:false` fallback returned an admitted outcome. Each pass used a distinct settled reservation/attempt in one causal call scope; the first remained durably `failed` with bounded metadata. Raw `message.thinking` was reduced to `thinking_observed` at the transport boundary and was absent from response debug output and durable failure reasons. Reviewer: Codex coordinator evidence review (not human approval). |
