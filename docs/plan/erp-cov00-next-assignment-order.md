...
Current disposition: `IMPLEMENTED` for COV-07a and COV-07b; runtime acceptance is pending. COV-07a moves Draft → Confirmed onto exact canonical readback of the same company-scoped MO id and preserves the BOM link. COV-07b extends that operator path through Confirmed → Progress → material consumption and closes three correctness defects found while certifying it: `consume_mo_materials` rediscovered a just-created raw move by `max(id)` instead of using returned identity; consumed raw moves stayed Assigned despite the quant effect already being applied; and `move_raw_count` incremented by one regardless of BOM-line count. The reducer now owns the exact inserted stock-move identity, closes every consumed raw move as Done, and stores the true relation count. The shared readback accepts material consumption only from `move_raw_ids` with one-for-one BOM-line product/UOM/quantity/location/state proof; replay and denial must preserve both that set and the component quantity. See [`erp-cov07a-mo-confirmation-status.md`](./erp-cov07a-mo-confirmation-status.md) and [`erp-cov07b-material-consumption-status.md`](./erp-cov07b-material-consumption-status.md).

Next bounded Manufacturing work is COV-07c: production quantity → finished-goods move → close, with exact `move_finished_ids`, destination-quant convergence, same-MO Done readback, and replay/stale/deny preservation. Keep work-order execution, quality, scrap/byproducts, and full costing separate.
