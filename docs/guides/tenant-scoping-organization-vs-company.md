...
Run `make codegen` after SpacetimeDB schema changes — it keeps `query-resource-row-type.json` and `stdb-generated-sql-columns.json` aligned between `frontend/packages/stdb/src/` and `crates/stdb-auth/assets/`.
