...
async fn executes_two_calls_then_returns_candidate_and_preserves_transcript() {
    ...
    let rec = recorder();
    let out = run_loop(7, &provider, &tools, rec.as_ref(), request(), limits())
        .await
        .expect("loop");
...
async fn run_zero_has_no_side_effects() {
    ...
    let rec = recorder();
    assert!(
        run_loop(0, &provider, &tools, rec.as_ref(), request(), limits())
            .await
            .is_err()
    );
...
async fn malformed_batch_denies_the_whole_batch() {
    ...
    let rec = recorder();
    let out = run_loop(1, &provider, &tools, rec.as_ref(), request(), limits())
        .await
        .expect("loop");
...
async fn recorder_failure_before_provider_is_returned() {
    ...
    assert!(run_loop(8, &provider, &tools, &rec, request(), limits())
        .await
        .is_err());
    assert_eq!(provider.request_count(), 0);
}
...
async fn recorder_failure_after_tool_prevents_next_provider_call() {
    ...
    assert!(run_loop(9, &provider, &tools, &rec, request(), limits())
        .await
        .is_err());
...
async fn provider_and_tool_errors_are_persisted_and_stop() {
    ...
    let rec = recorder();
    let out = run_loop(10, &provider, &tools, rec.as_ref(), request(), limits())
        .await
        .expect("loop");
    ...
    let rec = recorder();
    let out = run_loop(11, &provider, &tools, rec.as_ref(), request(), limits())
        .await
        .expect("loop");
...
async fn successful_events_are_contiguous_and_have_terminal_stop() {
    ...
    let rec = recorder();
    run_loop(12, &provider, &tools, rec.as_ref(), request(), limits())
        .await
        .expect("loop");
