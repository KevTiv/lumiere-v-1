...
fn seed_closed_period(ctx: &ReducerContext, fixture: &OrgFixture) -> Result<(), String> {
    ...
    let closed = ctx.db.account_period().id().find(&period_id).ok_or("Closed period disappeared")?;
    ...
    let after_replay = ctx.db.account_period().id().find(&period_id).ok_or("Period disappeared after replay")?;
