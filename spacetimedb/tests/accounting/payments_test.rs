...
pub fn test_payment_reconciles_invoice(ctx: &ReducerContext) -> Result<(), String> {
    ...
    let statement = ctx
        ...
        .iter()
        .find(|row| row.organization_id == org_id && row.name.as_deref() == Some("COV-08B exact statement"))
    ...
    let invoice_receivable_id = ctx
        ...
        .find(|line| line.account_internal_type.as_deref().is_some_and(|kind| kind.eq_ignore_ascii_case("receivable")))
        .map(|line| line.id)
    ...
    match reconcile_account_bank_statement_line(
        ...
        Err(error) if error.contains("already reconciled") => {}
        Err(error) => return Err(format!("unexpected bank reconciliation replay error: {error}")),
