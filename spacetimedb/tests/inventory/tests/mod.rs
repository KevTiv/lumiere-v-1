...
pub fn run_inventory_lot_validate_test(ctx: &ReducerContext) -> Result<(), String> {
    ...
    gap_fixes_test::test_outbound_transfer_keeps_lot_id(ctx)
        .map_err(|e| format!("outbound_lot_keep: {e}"))
}
...
#[spacetimedb::reducer]
pub fn run_inventory_integration_result_fk_relations_test(
    ctx: &ReducerContext,
) -> Result<(), String> {
    gap_fixes_test::test_integration_result_fk_relations(ctx)
        .map_err(|e| format!("integration_result_fk_relations: {e}"))
}
